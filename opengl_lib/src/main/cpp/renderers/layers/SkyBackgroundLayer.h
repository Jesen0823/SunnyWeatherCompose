 ...
 * 负责渲染天空背景，包括：
 * - 天空渐变（日/夜模式不同颜色）
 * - 太阳光晕效果（白天模式）：核心、内层光晕、外层光晕、太阳光线
 * - 月亮光晕效果（夜间模式）：多层光晕叠加，程序化生成月球表面陨石坑和阴影
 * - 天空模式调整（阴天变暗、雪天偏蓝、沙尘偏黄）
 * 
 * 实现原理：
 * 使用全屏四边形作为渲染载体，片段着色器根据位置计算天空渐变颜色。
 * 太阳/月亮区域使用 early discard 优化：远离光源的像素直接返回，避免不必要的光照计算。
 * 月球表面通过 6 个圆形区域模拟陨石坑，配合水平阴影增加立体感。
 * 
 * 参数配置：
 ...
 * - sunVisible: 太阳可见性（控制是否渲染太阳）
 * - skyMode: 天空模式（0=正常，1=阴雨，2=雪天，3=沙尘）
 */
